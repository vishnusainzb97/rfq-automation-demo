...
const SYSTEM_PROMPT = "You are an expert AI data extraction assistant for a Life Sciences company. Given an email containing a Request for Quote (RFQ) for chemical compositions, extract the chemical items listed and return ONLY a valid JSON array of objects. Do not wrap the JSON in Markdown formatting. Output exactly the raw JSON array. Each object in the array should conform to this structure: {\"Chemical Name\": \"Name of the chemical\", \"Qty\": \"Numerical quantity without text\", \"Units\": \"Units e.g. Kg or Ltr\", \"Unit Cost (INR)\": \"Price per unit as clean numeric value\", \"Source\": \"Name of the vendor or company\", \"Country\": \"Country of the vendor\", \"Lead Time\": \"Expected delivery time\"}. If a field is not present, use null for numerical fields or empty string for text fields.";
  ...
  const handleExtraction = async () => {
    ...
    try {
      ...
      for (let i = 0; i < MOCK_EMAILS.length; i++) {
        ...
        setStatus("Extracting data from email " + (i + 1) + " of " + MOCK_EMAILS.length + "...");
        ...
        try {
          ...
          console.error("JSON parse error for email " + (i + 1), cleaned);
        }
      }
      ...
      const enhancedData = allData.map((item) => {
        ...
        return {
          ...item,
          "Total Cost (INR)": Math.round(qty * unitCost * 100) / 100,
        };
      });

      setExtractedData(enhancedData);
      setStatus("Extraction Complete! " + enhancedData.length + " items found.");
    } catch (err: any) {
      console.error(err);
      ...
      <div className="w-full md:w-3/4 p-8 flex flex-col gap-8 h-screen overflow-y-auto">
        <div>
          ...
          <p className="text-gray-400 mt-2">
            Instant workflow automation. Extracts unstructured Request for Quote (RFQ) emails into a structured Excel sheet.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col gap-4">
            ...
            <button
              ...
              disabled={loading}
              className="mt-4 flex items-center justify-center gap-2 bg-gradient-to-r from-[#4facfe] to-[#00f2fe] text-white py-3 rounded-lg font-bold hover:opacity-90 transition-opacity disabled:opacity-50 cursor-pointer"
            >
              {loading ? (
            ...
            {status && <div className="text-sm text-center text-blue-300 mt-2">{status}</div>}
          </div>
            ...
            {extractedData.length > 0 ? (
              <div className="bg-[#1a1c23] rounded-lg overflow-hidden border border-[#333]">
                ...
                <div className="p-4 bg-[#2a2d36] border-t border-[#333] flex justify-between items-center">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-[#2ea043] text-white px-4 py-2 rounded font-semibold hover:bg-[#3fb950] transition-colors cursor-pointer"
                  >
              ...
              <div className="flex flex-col items-center justify-center p-12 bg-[#1a1c23] border border-[#333] rounded-lg border-dashed">
                ...
                <p className="text-gray-400 text-center">
                  Execute the AI extraction on the Inbox to view and download the structured RFQ table.
                </p>
              </div>
            )}
          </div>
